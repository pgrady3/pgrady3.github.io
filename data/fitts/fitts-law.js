...
function nextCondition() {
	if (conditionQueue.length === 0) {
		conditionQueue = [2, 3, 4];
		for (let i = conditionQueue.length - 1; i > 0; i--) {
			const j = Math.floor(Math.random() * (i + 1));
			[conditionQueue[i], conditionQueue[j]] = [conditionQueue[j], conditionQueue[i]];
		}
	}
	return conditionQueue.pop().toString();
}
